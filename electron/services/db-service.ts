...
export class DbService {
  ...
  async searchTablesInEnvironment(environmentId: string, query: string): Promise<TableSearchHit[]> {
    ...
    if (!trimmedQuery) {
      const fullResults = await Promise.all(
        connections.map(async (connection) => {
          try {
            const tables = await this.listTables(connection.id)
    ...
    const results = await Promise.all(
      connections.map(async (connection) => {
        try {
          const tables = await this.searchTables(connection.id, trimmedQuery)
          return tables.map((table) => ({
            connectionId: connection.id,
            connectionName: connection.name,
            engine: connection.engine,
    ...
    return rankTableSearchHits(results.flat(), trimmedQuery, 300)
  }
  ...
  private async searchPostgresTables(connection: ConnectionSummary, query: string): Promise<TableRef[]> {
    ...
    const compactQuery = compactTableSearchValue(trimmedQuery)
  ...
  private async searchSqliteTables(connection: ConnectionSummary, query: string): Promise<TableRef[]> {
    ...
    const compactTerm = compactTableSearchValue(trimmedQuery)
    ...
    return tables
      .filter((table) => {
        ...
        const compactTableName = compactTableSearchValue(table.name)
        const compactFqName = compactTableSearchValue(table.fqName)
        return (
          compactTableName.includes(compactTerm) ||
          compactFqName.includes(compactTerm) ||
          isSubsequenceMatch(compactTerm, compactTableName) ||
          isSubsequenceMatch(compactTerm, compactFqName)
        )
      })
      .slice(0, 180)
  }
